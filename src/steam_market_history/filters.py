...
_FIELD_ACCESSORS: dict[str, Callable[[Transaction], str]] = {
    ...
    # "" for a None category so a transaction with no category data simply
    # never matches any non-empty pattern, rather than raising on `.casefold()`.
...
def parse_query(text: str) -> Query:
    ...
    one of "game", "name", "acquisition", or "category". A leading `!` on
    a clause negates it. Patterns
